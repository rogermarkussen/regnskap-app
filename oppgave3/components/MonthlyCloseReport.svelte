<script>
  ...
  $: usesLegacySectionSnapshot = sections.length === 5
    && ['711', '712', '721', '731', '741'].every((value) => sections.includes(value));
...
<div class="report-shell">
  ...
  <section class="workspace">
    ...
    {#if usesLegacySectionSnapshot}
      <div class="legacy-data-warning" role="status">
        <strong>Datamappen bruker den gamle femseksjonsregelen.</strong>
        <span>Velg oppgave 3-mappen fra snapshot <code>2026-08-29-task3-sections-fixed</code> for å få alle seksjonene.</span>
      </div>
    {/if}
...
<style>
  ...
  .legacy-data-warning{display:grid;gap:3px;margin:-6px 0 16px;padding:10px 12px;border-left:3px solid var(--amber);background:#fff8ec;color:#725526;font-size:9px;line-height:1.45}.legacy-data-warning strong{font-size:10px}.legacy-data-warning code{font-size:9px}
